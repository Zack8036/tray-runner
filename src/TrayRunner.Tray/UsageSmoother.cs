...
/// <summary>
/// 對 1Hz 原始使用率取樣(CPU 或記憶體)套用指數移動平均(EMA)平滑:
/// <c>smoothed = α · 新取樣 + (1 − α) · 前一次 smoothed</c>。
///
/// 真實 CPU 使用率每秒尖刺嚴重,若直接驅動動畫會讓角色神經質地暴衝又龜速;記憶體較平穩
/// 但同樣受益於平滑以避免面板數字跳動。EMA 讓輸出朝新取樣平順地移動一小步,只需記住一個
/// 數字(前一次平滑值)。第一筆取樣直接作為種子,避免啟動時從假性低負載慢慢爬升。
///
/// 本類別為純邏輯、不具執行緒安全保證;預期由單一背景輪詢執行緒循序呼叫。
/// </summary>
public sealed class UsageSmoother
{
    ...
    /// </param>
    public UsageSmoother(double alpha = 0.3d)
    {
